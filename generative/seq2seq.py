...
@Desciption: Bert+decoder实现seq2seq任务     
@FilePath: /JDQA/generative/seq2seq.py
'''
...
class Seq2SeqModel(nn.Module):
    """
    Seq2Seq实现Nature Language Generate
    """
    ...
    def compute_loss(self, predictions, labels, target_mask):
        ...
        # 通过mask取消pad和句子a部分预测的影响
        return (loss(predictions, labels) * target_mask.float()).sum() / target_mask.float().sum()  
    ...
    def forward(self,
        ...
        # 传入输入，位置编码，token type id，还有句子a和句子b的长度，每次传入一个batch数据
        # 传入的几个值，在seq2seq的batch iter函数里面都可以返回
    ...
    def beam_search(self,
        ...
        for step in range(self.out_max_length):
            ...
            # 为啥有这个[], 就是因为要过滤掉结束的序列。
            # 用来保存新预测出来的一个字符，继续接到输入序列后面，再去预测新字符
            next_chars = []  
