...
class BertEmbeddings(nn.Module):
    """
    构建word embedding，positon embedding，token_type embedding 
    """
    ...
    def forward(self, input_ids=None, token_type_ids=None, position_ids=None):
        ...
        if position_ids is None:
            # position_ids: [0, seq_length)
            position_ids = torch.arange(seq_length,
                                        dtype=torch.long,
                                        device=device)
            # 处理成和input_shape一样的shape
        ...
        # 三个embedding相加为最终embedding
        embeddings = inputs_embeds + position_embeddings + token_type_embed
...
class BertSelfAttention(nn.Module):
    def __init__(self, config: BertConfig):
        super().__init__()
        # hidden_size 必须是multi head头数的整数倍
        ...
        # query, key, value从hidden_size升维为all_head_size
    ...
    def transpose_for_scores(self, x):
        ...
        拼接multi head的size
        """
        # x.size() + multi head size
        new_x_shape = x.size()[:-1] + (self.num_attention_heads,
                                       self.attention_head_size)
        x = x.view(*new_x_shape)

        # 最后的shape (batch_size, num_attention_heads, seq_len, head_size)
        return x.permute(0, 2, 1, 3)

    def forward(self, hidden_states, attention_mask, output_attentions=True):
        # 对hidden_states进行线性变换得到query, key, value
        ...
        # 计算attention score
        # (Q * K^T) / sqrt(attention_head_size)
        ...
        # 注意力加权
        # ((Q * K^T) / sqrt(attention_head_size)) * V
...
class BertAttention(nn.Module):
    """
    Self Attention + Attention Output
    """
    def __init__(self, config):
        super().__init__()
...
class BertEncoder(nn.Module):
    ...
    def forward(self,
        ...
        # 输入Bert时的hidden_states
        all_encoder_layers = [hidden_states]
        all_attention_matrices = [None]
...
class BertPooler(nn.Module):
    """
    BertEncoder中第一个token（[CLS]）对应的输出
    """
    def __init__(self, config):
        super().__init__()
        self.dense = nn.Linear(config.hidden_size, config.hidden_size)
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(self,
        ...
        if attention_mask is not None:
            # 如果传进来的注意力mask不是null，那就直接用传进来的注意力mask乘以原始mask
            # 原始mask是extended_attention_mask，这个是用来把pad部分置为0，去掉pad部分影响
        ...
        # 多层Bert Layer输出
        encoder_layers, attention_layers = self.encoder(
            embedding_output,
